...
class Net(nn.Module):
    def __init__(self, args, config):
        ...
        self.lp = Lap_Pyramid_Conv(num_high=1, in_chans=config.LP.in_chans)
    ...
    def recon(self, input_lp, denoise_high, x):
        assert (input_lp[0][0].shape == denoise_high.shape), print("input_lp[1][0].shape != denoise_high.shape")
    ...
    def forward(self, x, label):
        ...
        t = torch.randint(low=0, high=self.num_timesteps, size=(input_high0.shape[0] // 2 + 1,)).to(x.device)
        t = torch.cat([t, self.num_timesteps - t - 1], dim=0)[:input_high0.shape[0]].to(x.device)
        a = (1 - b).cumprod(dim=0).index_select(0, t).view(-1, 1, 1, 1)

        e = torch.randn_like(input_high0)
        ...
        if self.training:
            ...
            data_dict["pred_high0"] = denoise_high0
...
class LP_DDM(nn.Module):
    ...
    def estimation_loss_lp(self, output):
        ...
        # self.l2_loss(pred_LL, gt_LL)) 
        frequency_loss = 0.1 * (#self.l2_loss(input_high0, gt_high0) +
                                self.l2_loss(pred_high0, gt_high0)) +\
                         0.01 * (self.TV_loss(input_high0))
